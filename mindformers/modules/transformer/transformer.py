...
class CpmAttention(Cell):
    ...
    def construct(self, query_tensor, key_tensor, value_tensor, attention_mask, position_bias, key_past=None,
                  value_past=None, batch_valid_length=None):
        """Forward process of the MultiHeadAttention"""
        self._check_inputs(query_tensor, key_tensor, value_tensor, attention_mask, key_past,
                           value_past, batch_valid_length)
        ...
        # multi head attention considering attention mask
        # the return shape is [bs * seq_length, hidden_size]
        attention = self._attn(query, key, value, attention_mask, position_bias)
        # Output
        output = self.projection(attention)
        output = self.dropout(output)
        output = F.reshape(output, ori_shape)
    ...
    def _attn(self, query, key, value, attention_mask, position_bias):
        """
        Get the weighted score along the seq_length
        ...
        key = self.real_div(key, factor)
        score = self.batch_matmul(query, key)
        # position_bias: (batch, len_seq, len_seq)
        score = self.add_bias(score, position_bias)
